...
select accounts.account_no , clients.id, account_types.name from account_types, accounts left join account_types clients on clients.id = accounts.client_id ;
...
select transactions.id, transactions.amount, transactions_types.name from accounts, transactions_types right  join transactions on transactions.id = transactions.id ;

-- 4) Selecciona todos los tipos de transacciones  y  que coincidan con todas las transacciones realizadas, y su monto para que el resultado sea similar al de la siguiente tabla: 

select transactions.account_des , transactions.amount, transactions_types.name from accounts, transactions_types right  join transactions on transactions.id = transactions.id ;
